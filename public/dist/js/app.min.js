...
function validIntDesimal(f) {
        !(/^[0-9.]*$/i).test(f.value)?f.value = f.value.replace(/[^0-9.]/ig,''):null;
        } 

function validLaters(f) {
        !(/^[A-zÑñ]*$/i).test(f.value)?f.value = f.value.replace(/[^A-zÑñ \n,()%+./-]/ig,''):null;
} 

function validLatesNumber(f) {
        !(/^[A-zÑñ0-9]*$/i).test(f.value)?f.value = f.value.replace(/[^A-zÑñ0-9 \n,()%+./-]/ig,''):null;
} 

function checkFormDate(f)
  {
    // regular expression to match required date format
    re = /^(\d{1,2})\/(\d{1,2})\/(\d{4})$/;

    if(f.value != '') {
      if(regs = f.value.match(re)) {
       ...
       /* if(regs[3] < 1902 || regs[3] > (new Date()).getFullYear()) {
        ...
        alert("Invalid date format: " + f.value);
        f.focus();
        return false;
      }
    }}
